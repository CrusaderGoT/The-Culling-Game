...
class BaseUser(SQLModel):
    ...
    email: EmailStr = Field(
        index=True, unique=True, description="the email address of the user"
    )
...
class BaseCT(SQLModel):
    ...
    name: str = Field(
        min_length=3,
        max_length=100,
        description="The name of the cursed technique, must be between 3 and 100 characters",
        schema_extra={
            "examples": ["Boogie Woogie", "Cursed Speech", "Infinity", "Black Flash"]
        },
    )
    definition: str = Field(
        min_length=50,
        max_length=500,
        description="The definition of the cursed technique, explaining what it does without including its subsets",
    )
...
class BaseCTApp(SQLModel):
    ...
    name: str = Field(
        min_length=3,
        max_length=100,
        description="The name of the application, must be between 3 and 100 characters",
        schema_extra={
            "examples": ["Hollow Purple", "Demon Dogs", "Resonance", "Overtime"]
        },
    )
    application: str = Field(
        min_length=100,
        max_length=500,
        description="The application of the cursed technique, explaining how it is used or applied in practice",
    )
...
class BasePermission(SQLModel):
    ...
    model: ModelName = Field(
        description="The model the permission applies to", index=True
    )
...
class BaseBarrierTech(SQLModel):
    ...
    domain_expansion: bool = Field(
        default=False, description="the player's domain expansion"
    )
    binding_vow: bool = Field(default=False, description="the player's binding vow")
    simple_domain: bool = Field(default=False, description="the player's simple domain")
    ...
    de_end_time: datetime | None = Field(
        default=None,
        sa_column=Column(TIMESTAMP(timezone=True)),
        description="the time a player cast their domain",
    )
    bv_end_time: datetime | None = Field(
        default=None,
        sa_column=Column(TIMESTAMP(timezone=True)),
        description="the time a player cast their binding_vow",
    )
    sd_end_time: datetime | None = Field(
        default=None,
        sa_column=Column(TIMESTAMP(timezone=True)),
        description="the time a player cast their simple_domain",
    )
...
class ActionTimePoint(SQLModel):
    ...
    match_duration: timedelta = timedelta(seconds=10)
