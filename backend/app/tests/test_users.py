...
def test_create_user(test_client: TestClient):
    ...
    # Only compare keys that exist in both payload and response
    common_keys = set(payload_data.keys()) & set(res_keys)
    for k in common_keys:
        if k == "country":
            assert Country(response_data[k]) == payload_data[k]
        else:
            assert response_data[k] == payload_data[k]
...
def test_current_user(authenticated_test_client: tuple[TestClient, dict]):
    ...
    assert (
        ...
        and type(response.json()["created"]) is str
    )
...
def test_a_user(authenticated_test_client: tuple[TestClient, dict]):
    ...
    # check database calculated fields was made
    assert (
        response.json()["id"] == auth_user["id"] and type(response.json()["id"]) is int
    )
    assert (
        response.json()["created"] is not None
        and type(response.json()["created"]) is str
...
def test_edit_user(authenticated_test_client: tuple[TestClient, dict]):
    ...
    username = gen_rand_username()
    email = gen_rand_email()
    edit_user_payload = EditUser(username=username, email=email, country=Country("JP"))
    response = authenticated_test_client[0].patch(
    ...
    assert Country(res_data["country"]) == edit_data["country"]
