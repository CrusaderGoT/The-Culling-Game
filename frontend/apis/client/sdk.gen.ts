...
export class Users {
     ...
     * Verify User
     */
    public static verifyUser<ThrowOnError extends boolean = false>(options?: Options<VerifyUserData, ThrowOnError>) {
        return (options?.client ?? client).post<VerifyUserResponses, VerifyUserErrors, ThrowOnError>({
            requestValidator: async (data) => await zVerifyUserData.parseAsync(data),
            security: [{ scheme: 'bearer', type: 'http' }],
            url: '/users/verify',
            ...options
        });
